  ...
  return rows
    .map((row, index) => {
      ...
      return {
        ...
        category: String(row.category || 'Coding & Tech'),
        educator: String(row.educator || ''),
        meta: '',
        price: Number.isFinite(price) ? price : 0,
        palette: ['teal','orange','purple','blue'][index % 4],
        rating: Math.min(5, Math.max(4, rating)).toFixed(1),
        reviews: Number(row.review_count || 0),
        likes: 0,
        dislikes: 0,
        thumbnail_url: row.thumbnail_url ? String(row.thumbnail_url) : null,
        telegram_url: row.telegram_url ? String(row.telegram_url) : null,
        is_published: row.is_published !== false,
      } satisfies LiveCourse;
    })
    .filter((course): course is LiveCourse => Boolean(course));
}
...
export function subscribeLiveChanges(onChange: () => void) {
  if (!isLiveBackendConfigured() || typeof window === 'undefined') return () => {};
  try {
    realtimeClient ??= createRealtimeClient(SUPABASE_URL!, SUPABASE_PUBLISHABLE_KEY!, {
      auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
    });
    let triggerTimer = 0;
    const trigger = () => {
      window.clearTimeout(triggerTimer);
      triggerTimer = window.setTimeout(onChange, 180);
    };
    const channel = realtimeClient
      .channel('sayeed-courses-live')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'course_catalogue' }, trigger)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'course_reactions' }, trigger)
